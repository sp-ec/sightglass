...
export const upsertGamesByTag = async (tag: string, appTags: Record<string, Record<string, number>>) => {
  ...
  for (let i = 0; i < entries.length; i += CHUNK_SIZE) {
    ...
    for (const [appId, detailsAny] of chunk) {
      ...
      const owners = typeof details.owners === 'string' ? details.owners : (details.owners_range || '0 .. 0');
      const [owners_min, owners_max] = parseRange(owners);
      const price = typeof details.price === 'number' ? toDecimal(details.price) : null;
      const ccu = details.ccu ?? null;
      const developer = details.developer ?? null;
      const publisher = details.publisher ?? null;

      params.push(appId, positive, negative, owners_min, owners_max, price, ccu, developer, publisher, [tag], new Date());
    }
