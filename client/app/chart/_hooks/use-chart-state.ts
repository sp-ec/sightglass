...
export function useChartState() {
    ...
    const [aggregateMode, setAggregateMode] = useState<"average" | "median">("average");
    const [sortingMode, setSortingMode] = useState<"flat" | "ascending" | "descending">("flat");
    const [chartData, setChartData] = useState<ChartResponse | null>(null);
    const [sortedChartData, setSortedChartData] = useState<ChartResponse | null>(null);
    ...
    const axisOptions = chartType === "scatter" ? SCATTER_NUMERIC_AXES : AXIS_OPTIONS;
    ...
    const bucketDisplayValue = useMemo(() => {
        ...
        return bucketConfig.displayValue ? bucketConfig.displayValue(parsed) : bucketSize;
    ...
    useEffect(() => {
        ...
        if (sortingMode === "ascending") {
            sortedPoints.sort((a, b) => (Number(a[yAxis]) || 0) - (Number(b[yAxis]) || 0));
        } else if (sortingMode === "descending") {
            sortedPoints.sort((a, b) => (Number(b[yAxis]) || 0) - (Number(a[yAxis]) || 0));
        }
    ...
    return {
        state: {
            groupBy, chartType, xAxis, yAxis, bucketSize, aggregateMode,
            sortingMode, tagsCounted, loading, error, bucketError, bucketConfig,
            axisOptions, selectedGroupByLabel, selectedChartTypeLabel, xAxisLabel,
            selectedYAxisLabel, xAxisDisabled, bucketDisplayValue, sortedChartData,
            canRenderChart: Boolean(groupBy && chartType)
        },
        actions: {
            setGroupBy, setChartType, setXAxis, setYAxis, setBucketSize,
            setAggregateMode, setSortingMode, setTagsCounted, setBucketError
        }
    };
}